...
void draw() {
  ...
  if (estado.equals("interaccion")) {
    ...
    if (haySonido && !hayRuido) { // --- Siempre que se detecte cualquier sonido se va a dibujar un trazo (los trazos son las pinceladas grandes)
      ...
      if (tf.size() <= 8) {
        tf.add(new Trazo(p, random(width), random(height), pitchtinte, floor(random(20)), ampEscala, random(1, 10), radians(random(-2, 2))));
      }
    }
    if (terminoElSonido) {
      ...
      if (momentoActual < marcaDeTiempo + umbralDeTiempo) { // --- Si el sonido es corto se dibuja una linea/garabato
        ...
        ls.add(new Linea(ptrazos, random(width), random(height), floor(random(140, 255)), floor(random(19)), random(10), radians(random(-6, 6))));
      }
    }
  }
  ...
  if (!haySonido && brillo > UMBRAL_BRILLO) { // --- Si se detecta un sonido con un brillo > 900, se actualiza la dirección y velocidad de los trazos/garabatos
    for (Trazo t : ts) {
